...
function mergeStationsWithPrices(stations, priceRecords) {
  ...
  const withPricesCount = out.filter(s => s.prices.length > 0);
  const counts = { E10: 0, E5: 0, B7: 0, SDV: 0, B10: 0, HVO: 0 };
  withPricesCount.forEach(s => s.prices.forEach(p => {
    if (counts[p.fuelType] !== undefined) counts[p.fuelType]++;
  }));
  console.log(`📊  UK fuel coverage:  E10=${counts.E10}  E5=${counts.E5}  B7=${counts.B7}  B7 Premium=${counts.SDV}  B10=${counts.B10}  HVO=${counts.HVO}`);
...
const CANONICAL_HOST = process.env.CANONICAL_HOST || 'fuelfindermap.com';
...
app.get('/api/prices', async (req, res) => {
  ...
  try {
    ...
    console.log(`📦  Served ${data.stations.length} UK stations\n`);
    res.json(data);
...
refreshCache()
  .then(d => console.log(`🔥  Cache warmed – ${d.stations.length} UK stations ready`))
